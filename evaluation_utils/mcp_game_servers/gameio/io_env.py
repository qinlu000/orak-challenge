...
from .gui_utils import get_named_windows, get_named_windows_fallback, get_screen_size, mouse_button_down, mouse_button_up, key_down, key_up, mouse_wheel_scroll, type_keys, mouse_click, get_mouse_location, mouse_move_to

# TODO: remove all config, logger dependency here
...
class IOEnvironment():
    ...
    def list_session_screenshots(self, session_dir: str = None):
        if session_dir is None:
            session_dir = self.config.work_dir
    ...
    def mouse_move_normalized(self, x, y, relative = False, from_center = False):
        ...
        w, h = self.config.env_resolution
    ...
    def mouse_move(self, x, y, duration = -1, relative = False):
        mouse_move_to(x, y, duration, relative, self.screen_resolution, self.config.env_region)
    ...
    def clip_check_horizonal_angle(self, theta):
        ...
        if pixels > 0 and mx + pixels > self.config.env_resolution[0]:
            result = True
    ...
    def get_windows_by_config(self):
        return get_named_windows_fallback(self.config.env_name, self.config.win_name_pattern)
