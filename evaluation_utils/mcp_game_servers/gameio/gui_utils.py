...
class TargetWindow():
    ...
    def get_client_region(self, win_resolution: Optional[List[int]] = None) -> Tuple[int, int, int, int]:
        """
        Get the client region information (left, top, width, height)
        """
        if _isWin():
            ...
            '''
            result = self._run_applescript(applescript_command, capture_output=True).strip()
            if result == "fullscreen":
                w, h = get_screen_size()
                return (0, 0, w, h)

            left, top, width, height = tuple(map(int, result.split(",")))
            if win_resolution is not None:
                top += height - win_resolution[1]
                width = win_resolution[0]
                height = win_resolution[1]
